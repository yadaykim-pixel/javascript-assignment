...
function renderMovies(movies) {
  container.textContent = ""; // Loading 상태 지우기
...
async function getPopularMovies() {
  ...
  try {
    ...
    // TODO 4. response.ok가 false라면 오류 안내 문구를 표시하고 return으로 함수를 종료하세요. (HTTP 오류 처리)
    if (!response.ok) {
    ...
    container.innerHTML = "영화 정보를 불러오지 못했습니다.";

    // TODO 9. console.error(error)로 실제 오류를 출력하세요. (이미 위에서 실행됨)
  }
}
